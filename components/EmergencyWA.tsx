...
const EmergencyWA = () => {
  ...
  return (
    <AnimatePresence>
      {!isFooterVisible && (
        <motion.div
          // AREA TANGKAP: Seluas layar secara vertikal (h-screen) di sisi kanan
          drag="x"
          dragConstraints={{ left: -100, right: 100 }}
          dragElastic={0.05}
          onDragStart={() => setShowTooltip(false)}
          onDragEnd={(_, info) => {
            // Sensitivitas tinggi untuk swipe di mana saja di area kanan
            ...
            <AnimatePresence>
              {showTooltip && !isHiddenBySwipe && (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  className="absolute right-[120%] bg-black/80 text-white text-[10px] py-1.5 px-3 rounded-lg whitespace-nowrap"
                >
                  ← Swipe kiri/kanan di sini
                </motion.div>
              )}
            </AnimatePresence>
