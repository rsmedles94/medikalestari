...
const MobileSearchModal: React.FC<MobileSearchModalProps> = memo(
  ({ isOpen, onClose }) => {
    ...
    const randomCategories = useMemo(() => {
      if (!isOpen) return [];
      return SPECIALTY_CATEGORIES.filter((item) => item !== "Semua Spesialis");
    }, [isOpen]);
    ...
    const saveToHistory = useCallback((doctor: Doctor) => {
    ...
    const removeHistoryItem = useCallback((id: string | number) => {
      setDoctorHistory((prev) => {
    ...
    const handleNavigation = useCallback(
      ...
      [saveToHistory, router, handleCloseModal],
    );
                    ...
                    {searchResults.map((doctor) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] text-[#003f88] font-semibold">
                            {doctor.specialty}
                          </p>
                        </div>
